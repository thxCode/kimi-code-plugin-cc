...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.sessionId) {
    lines.push(`  Kimi session ID: ${job.sessionId}`);
  }
  ...
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    ...
    lines.push("  Stricter review: /kimi:adversarial-review --wait");
  }
...
export function renderNativeReviewResult(result, meta) {
  ...
  if (stdout) {
    ...
    lines.push("Kimi review completed without any stdout output.");
  } else {
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const resumeCommand = sessionId ? `kimi -r ${sessionId}` : null;
  if (isStructuredReviewStoredResult(storedJob) && storedJob?.rendered) {
    const output = storedJob.rendered.endsWith("\n") ? storedJob.rendered : `${storedJob.rendered}\n`;
    if (!sessionId) {
      return output;
    }
    return `${output}\nKimi session ID: ${sessionId}\nResume in Kimi: ${resumeCommand}\n`;
  }

  const rawOutput =
    (typeof storedJob?.result?.rawOutput === "string" && storedJob.result.rawOutput) ||
    (typeof storedJob?.result?.kimi?.stdout === "string" && storedJob.result.kimi.stdout) ||
    "";
  if (rawOutput) {
  ...
  if (sessionId) {
    lines.push(`Kimi session ID: ${sessionId}`);
    lines.push(`Resume in Kimi: ${resumeCommand}`);
  }
